...
### 💡 The Solution
RTK solves this by intercepting commands, stripping the noise, caching the raw data in a local FTS5 vector database, and returning only the pure semantic signal. By enforcing YAGNI developer behaviors and compressing outputs, the toolkit saves an **average of ~81% of tokens** across verified scenarios (ranging from 27% to 97%).
...
## ✨ Features & Benchmarks — Average **81.8%** Token Savings
...
### 📥 Phase 1: Input Virtualization (What the AI reads) — Avg **81.9%**
...
| :--- | :--- | ---: | ---: | :--- |
| 🦀 **Cargo Test** | `rtk cargo test` (REAL, 217 tests) | 4,097 | **616** | **📉 85.0%** |
...
| 📜 **Git Log** | `rtk git log` (REAL, 15 commits) | 4,600 | **338** | **📉 92.7%** |
...
| 🐍 **Ruff** | `rtk ruff check` (12 violations) | 511 | **242** | **📉 52.6%** |
| 🐍 **Mypy** | `rtk mypy --pretty` (10 errors) | 335 | **203** | **📉 39.4%** |
| 🐍 **Pip Install** | `rtk pip install` (8 new, 15 cached) | 1,155 | **73** | **📉 93.7%** |
| 🟦 **ESLint** | `rtk eslint` (8 problems, stylish) | 196 | **144** | **📉 26.5%** |
| 🟦 **tsc** | `rtk tsc --pretty` (5 errors) | 456 | **125** | **📉 72.6%** |
| 🟦 **Vitest** | `rtk vitest run` (2 failures) | 351 | **156** | **📉 55.6%** |
| 🐙 **gh pr checks** | `rtk gh pr checks` (6 checks) | 206 | **74** | **📉 64.1%** |
| 🐳 **Docker Build** | `rtk docker build` (8 layers) | 2,506 | **186** | **📉 92.6%** |
| 🐳 **Docker ps** | `rtk docker ps` (6 containers) | 318 | **172** | **📉 45.9%** |
...
| 🗜️ **Context Pack** | `rtk pack --strip --skeleton` (REAL) | 39,925 | **7,031** | **📉 82.4%** |
...
| 🔒 **DLP Redaction** | Auto-redacts API keys (OpenAI/Anthropic/Stripe/AWS/GitHub/GitLab/npm/HF/GCP), JWT, PEM keys, DB credentials, high-entropy secrets. | **📉 41.9%** |
...
*   **Input Wrappers (20 tools)**: `rtk git status/diff/log`, `rtk cargo test/build/check`, `rtk pytest`, `rtk ruff check`, `rtk mypy`, `rtk pip install`, `rtk eslint`, `rtk tsc`, `rtk vitest`, `rtk docker build/run/ps`, `rtk gh pr checks`, `rtk npm`, `rtk yarn`, `rtk pnpm`, `rtk composer`, `rtk terraform`, `rtk dotnet`, `rtk gradle`, `rtk go_test`, `rtk ls`.
